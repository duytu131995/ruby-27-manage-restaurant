class DinnerTablesController < ApplicationController
  ...
  def show
    if params[:search]
      @dishes = Dish.in_stock.searchDish("%#{params[:search]}%").page(params[:page]).per Settings.dinner_tables.show
      @count = Dish.in_stock.searchDish("%#{params[:search]}%").count
    else
      @dishes = Dish.in_stock.page(params[:page]).per Settings.dinner_tables.show
